...
httpfile = response.read().decode('utf-8').split('\n')
...
for log in httpfile:
    ...
    if len(log) == 10:
      ...
      #parse date using strptime()method
        ...
        if count_dict.get(date.tm_mon):
          #current month/week check
            if count_dict[date.tm_mon].get(week):
              #if true check day
                if count_dict[date.tm_mon][week].get(date.tm_mday):
                    count_dict[date.tm_mon][week][date.tm_mday] += 1
...
for month in sorted(count_dict):
    for week in sorted(count_dict[month]):
        for day in sorted(count_dict[month][week]):
            print ('Month:', month, 'day:', day, '-', count_dict[month][week][day])
print ()

for month in sorted(count_dict):
    for week in sorted(count_dict[month]):
        print ('Week:', week, '-', sum(count_dict[month][week].values()))
print ()

for month in sorted(count_dict):
    monthsum = 0
    for week in sorted(count_dict[month]):
        monthsum += sum(count_dict[month][week].values())
...
mostrequested = sorted(request_file.items(), key=lambda x: x[1], reverse = True)[0][0]
leastrequested = sorted(request_file.items(), key=lambda x: x[1])[0][0]
